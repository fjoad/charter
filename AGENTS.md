...
**Tier-2 recovery (v0.4.0+):** If `/charter-recover` isn't enough (CONTEXT.md was sparse, the user emphasized something not captured, etc.), `/charter-replay` runs `scripts/replay-filter.py` over the session's JSONL — keeping genuine human prompts + assistant text, excluding tool I/O, thinking, sidechain, and harness injections (task notifications, slash-command echoes, interrupt markers, compaction summaries, image placeholders). As of v0.7.0 it's a one-shot: `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/replay-filter.py"` with no args auto-finds the current session transcript and prints the dialogue to stdout (counts to stderr). Never read the raw `.jsonl` directly; that's tier-3 and a documented anti-pattern.
